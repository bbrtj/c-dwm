...
#define MIN_MFACT 0.2
#define MAX_MFACT 0.8
...
enum {
	...
	SchemeTitleDim, // \x04
	SchemeTitleSel, // \x05
	SchemeTitleSuccess, // \x06
	SchemeTitleAlert, // \x07
	SchemeTitleUrgent // \x08
};
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTitleSel : SchemeTitleDim]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
		x += w;
	}
	w = blw = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeTitleDim]);
...
incnmaster(const Arg *arg)
{
	...
	selmon->nmaster = selmon->pertag->nmasters[selmon->pertag->curtag] =
		MIN(n, MAX(selmon->nmaster + arg->i, 0));
	arrange(selmon);
}
...
updatesystray(void)
{
	...
	unsigned int x = m->mx + m->mw - (sw + lrpad / 2);
	unsigned int w = 1;
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		XMoveResizeWindow(dpy, i->win, i->x, systrayspacing, i->w - systrayspacing * 2, i->h - systrayspacing * 2);
		w += i->w - systrayspacing * 2;
		if (i->mon != m)
			i->mon = m;
	}
